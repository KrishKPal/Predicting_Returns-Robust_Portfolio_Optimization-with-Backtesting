...
def visualize_single_portfolio(mu, Sigma, w, tickers, title_suffix=""):
    """
    Visualize inputs and diagnostics for a single portfolio.

    mu : array of shape (n_assets,)
    Sigma : array of shape (n_assets, n_assets)
    w : array of shape (n_assets,)
    ...
    plt.bar(tickers, w)
    plt.ylabel("Weight")
    plt.title(f"Final portfolio weights{title_suffix}")
    ...
    sharpe = exp_return / (volatility + 1e-8)
